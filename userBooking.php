...
include "db_connect.php";
$sql = "SELECT * FROM bookings WHERE email='$email' ORDER BY booking_id DESC";
...
<div>
    <th><a href="homepage.php"><button class="button button1">Home</th></button></a></th>
  ...
  <?php while($admin = $sqlQuery->fetch_assoc()):
    ...
    <td><?=$time?></td>
    <td> P <?=$toPay?></td>
    <form action="bookingCancel.php" method="post">  
    ...
    <td><button type="submit" id="deny" name="cancel">Cancel</button></td>
    </form>
  </tr>
  <?php endwhile;?>
</table>
